  ...
  async addTextOverlayToVideo(videoPath, transcriptionData, textStyle, outputDir, subtitlePositions = null) {
    ...
    return new Promise((resolve, reject) => {
      ...
      if (isDev) {
        ...
        overlayScript = path.join(process.cwd(), 'src', 'python', 'transcription', 'overlay_subtitles.py');
      } else {
        // Production: use resourcesPath
        overlayScript = path.join(process.resourcesPath, 'python', 'transcription', 'overlay_subtitles.py');
      }

      console.log(`[VIDEO OVERLAY] Development mode: ${isDev}`);
      console.log(`[VIDEO OVERLAY] Using script path: ${overlayScript}`);
      console.log(`[VIDEO OVERLAY] process.resourcesPath: ${process.resourcesPath}`);
      ...
      const args = [
        ...
        outputPath,
        '--text-style', JSON.stringify(textStyle || {}),
        '--subtitle-positions', JSON.stringify(subtitlePositions || [])
      ];

      console.log(`[VIDEO OVERLAY] Running: python ${args[0]} [transcription-data] ${args[2]} ...`);
