...
class VideoProcessor:
    ...
    def create_clip(self, video_path: str, start_time: float, end_time: float,
        ...
        if output_path is None:
            ...
            output_path = self.temp_dir / \
                f"{video_name}_clip_{int(start_time)}_{int(end_time)}.mp4"
        ...
        try:
            ...
            if target_aspect in ['9:16', '1:1']:
                ...
                scale_factor = max(target_width / clip.w,
                                   target_height / clip.h)
    ...
    def add_captions_with_style(self, video_path: str, captions_data: list,
        ...
        try:
            ...
            print(
                f"Text style - fontsize: {fontsize}, color: {color}, position: {position}")
            ...
            for caption in captions_data:
                try:
                    ...
                    print(
                        f"Method 1 failed for '{caption['text'][:50]}...': {text_error}")
                    try:
                        ...
                        text_clips.append(txt_clip)
                        print(
                            f"Created text clip (method 2): '{caption['text'][:50]}...'")

                    except Exception as text_error2:
                        print(
                            f"Method 2 also failed for '{caption['text'][:50]}...': {text_error2}")
...
def main():
    ...
    parser.add_argument('--start', type=float,
                        help='Start time for clip creation')
    ...
    parser.add_argument('--font-color', default='white',
                        help='Font color for captions')
    parser.add_argument('--stroke-color', default='black',
                        help='Stroke color for captions')
    parser.add_argument('--stroke-width', type=int,
                        default=2, help='Stroke width for captions')
    ...
    try:
        if args.command == 'info':
            ...
            clip_path = processor.create_clip(
                args.video_path, args.start, args.end, args.output)
            print(f"Clip created: {clip_path}")

        elif args.command == 'thumbnail':
            thumbnail_path = processor.generate_thumbnail(
                args.video_path, output_path=args.output)
