  ...
  useEffect(() => {
    ...
    const handleTimeUpdate = () => {
      ...
      if (showSubtitlePreview && transcriptionResult?.transcription?.segments) {
        const currentSub = transcriptionResult.transcription.segments.find(segment => 
    ...
    try {
      ...
      try {
        ...
        if (bufferResult.success) {
          ...
          console.log('Setting blob video URL:', blobUrl);
          setBlobUrl(blobUrl);
        ...
        console.warn('Blob URL approach failed, trying secure protocol:', blobError);
        ...
        if (urlResult.success) {
          ...
          throw new Error(`Both approaches failed. Blob: ${blobError.message}, Protocol: ${urlResult.error}`);
        }
      }
    } catch (error) {
      console.error('Video preview preparation failed:', error);
      setPreviewError('Unable to prepare video preview. Processing will still work.');
    } finally {
      ...
      <div className="space-y-4">
        <div className="relative bg-black rounded-lg overflow-hidden">
          {isLoadingPreview ? (
            ...
            <div className="relative w-full h-64">
              ...
              {showSubtitlePreview && currentSubtitle && !showSubtitleOverlay && (
                <div
              ...
              {showSubtitleOverlay && (
                <div className="absolute top-2 left-2 bg-red-900 bg-opacity-75 text-white px-2 py-1 rounded text-xs z-20">
                  Debug: Overlay={showSubtitleOverlay ? 'ON' : 'OFF'}, Data={subtitleOverlayData.length} subtitles
                </div>
              )}
        ...
        <div className="pt-4 border-t border-gray-700 space-y-3">
          ...
          {onSubtitleOverlayToggle && subtitleOverlayData.length > 0 && (
            ...
            >
              ...
              {showSubtitleOverlay ? 'Hide Subtitle Editor' : 'Edit Subtitle Positions'}
            </button>
          )}
        ...
        <div className="text-sm text-gray-400">
          ...
          {videoInfo && (
            <div className="mb-2">
              <p className="mb-1">
                <strong>Resolution:</strong> {videoInfo.width}×{videoInfo.height}
              </p>
              <p className="mb-1">
                <strong>Aspect Ratio:</strong> {videoInfo.aspectRatio.toFixed(2)} {
                  videoInfo.isVertical ? '(Vertical)' : 
                  videoInfo.isSquare ? '(Square)' : 
                  videoInfo.isWidescreen ? '(Widescreen)' : '(Standard)'
                }
              </p>
            </div>
          )}
          ...
          {showSubtitlePreview && transcriptionResult && (
            <div className="mt-4 p-3 bg-blue-900/20 border border-blue-700 rounded-lg">
              ...
              <p className="text-blue-200 text-xs mb-3">
                Play the video to see subtitles positioned as they will appear in the final output.
                Subtitles are automatically optimized for {videoInfo?.isVertical ? 'vertical' : videoInfo?.isSquare ? 'square' : 'standard'} aspect ratio.
              </p>
              <button
                onClick={downloadSRT}
